...
// the Player.
// Entity 수퍼클래스를 충분히 활용하는 유일한 서브클래스다
// (맵에 기초한 타일 안에서 움직임을 요구하는 클래스가 더이상 없다)
// 플레이어에게 연관된 모든 게임 플레이를 담고 있다
...
public class Player extends Entity {
	...
	// 자기 자신은 화살표로 표시
	private BufferedImage[] arrow = Content.ARROW[0];
...
//		super.draw(g);
		...
		// 자기 자신이라면 화살표로 표시
		if(UDP_Client.id.contains(nickname))
			g.drawImage(arrow[0], x + xmap - width / 2 + 6, y + ymap - height / 2 - 8, null);
		
		// 상대방이라면 아이디를 그려줌
		else
			Content.drawString(g, nickname, x + xmap - width / 2 - nickname.length()*3, y + ymap - height / 2 - 10);
	}
