...
//메인 플레잉 게임상태
//너가 게임플레이하는데 필요한 모든 것을 담고있다 
//플레이어, 타일맵, 다이아몬드 등...
//모든 게임 오브젝트들을 업데이트하고 그린다
...
import com.neet.DiamondHunter.Main.ChatPanel;
...
public class PlayState extends GameState {
	...
	public void update() {
		...
		if (player.numDiamonds() + player2.numDiamonds() == player.getTotalDiamonds()) {
		...
		for (int i = 0; i < items.size(); i++) {
			...
			if (player.intersects(item)) {
				if ((item.getType() == Item.AXE && player.hasAxe() == false)
						|| (item.getType() == Item.BOAT && player.hasBoat() == false)) {
					items.remove(i);
					i--;
			...
			if (player2.intersects(item)) {
				if ((item.getType() == Item.AXE && player2.hasAxe() == false)
						|| (item.getType() == Item.BOAT && player2.hasBoat() == false)) {
					items.remove(i);
					i--;
	...
	public void handleInput() {
		...
		if (Keys.isPressed(Keys.ESCAPE)) {
			...
			gsm.setPaused(true);
			String msg = null;
			msg = String.format("%s\t%s\t%s\t","cmd",Game.client.id,"pause");
			Game.client.send_Message(msg);
		}
		if (blockInput)
			return;
		// 이벤트 발생 시 좌표값을 서버로 보냄.
		if (Keys.isDown(Keys.LEFT)) {
			player.setLeft();
			cmdCount++;
			String msg = null;
			msg = String.format("%s\t%s\t%s\t","cmd",Game.client.id,"left");
			Game.client.send_Message(msg);
			try {
				Thread.sleep(10);
		...
		if (Keys.isDown(Keys.RIGHT)) {
			...
			cmdCount++;
			String msg = null;
			msg = String.format("%s\t%s\t%s\t","cmd",Game.client.id,"right");
			Game.client.send_Message(msg);
			try {
				Thread.sleep(10);
			} catch (InterruptedException e) {
		...
		if (Keys.isDown(Keys.UP)) {
			...
			cmdCount++;
			String msg = null;
			msg = String.format("%s\t%s\t%s\t","cmd",Game.client.id,"up");
			Game.client.send_Message(msg);
			try {
				Thread.sleep(10);
			} catch (InterruptedException e) {
		...
		if (Keys.isDown(Keys.DOWN)) {
			...
			cmdCount++;
			String msg = null;
			msg = String.format("%s\t%s\t%s\t","cmd",Game.client.id,"down");
			Game.client.send_Message(msg);
			try {
				Thread.sleep(10);
			} catch (InterruptedException e) {
		...
		if (Keys.isPressed(Keys.SPACE)) {
			player.setAction();
			String msg = null;
			msg = String.format("%s\t%s\t%s\t","cmd",Game.client.id,"act");
			Game.client.send_Message(msg);
			try {
				Thread.sleep(10);
			} catch (InterruptedException e) {
