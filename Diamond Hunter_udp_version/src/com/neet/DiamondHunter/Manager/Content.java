...
public class Content {
	...
	public static BufferedImage[][] MENUBG = load("/HUD/menuscreen.gif", 128, 159);
	...
	public static BufferedImage[][] ARROW = load("/HUD/arrow.gif", 5, 6);
	...
	// 플레이어 2의 폰트
	public static BufferedImage[][] font2 = load("/HUD/font2.gif", 8, 8);
	...
	public static void drawString2(Graphics2D g, String s, int x, int y) {
		...
		for(int i = 0; i < s.length(); i++) {
			...
			if(c >= 65 && c <= 90) c -= 65; // letters
			if(c >= 48 && c <= 57) c -= 22; // numbers
			int row = c / font2[0].length;
			int col = c % font2[0].length;
			g.drawImage(font2[row][col], x + 8 * i, y, null);
		}
	}
}
